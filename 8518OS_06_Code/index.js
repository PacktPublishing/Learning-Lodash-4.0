...
function getIngredients(recipe, ingredients, recipeIngredients) {
  ...
  return _(ingredients)
    .filter(function(ingredient) {
      return _.find(relIngredients, {ingredient_id: ingredient.id});
    })
    .map(function(ingredient) {
      return _.assign(ingredient, _.omit(_.find(relIngredients, {ingredient_id: ingredient.id}), ['ingredient_id']));
    })
    .value();
}
...
router.get('/', function (req, res, next) {
  ...
  res.json(_(recipes)
    ...
    .map(function (recipe) {
      if (relation) {
        recipe[expand] = _(relation).findWhere({ id: recipe[expand + 'Id'] });
...
router.get('/:id', function (req, res, next) {
  var db = new JsonDB('db', false, false);
  var recipes = db.getData('/recipes');
  var recipe = _.findWhere(recipes, {id: _.parseInt(req.params.id)});
  if (recipe) {
    recipe.user = _.findWhere(db.getData('/users'), {id: recipe.userId});
...
router.post('/', auth.token, function(req, res, next) {
  ...
  try {
    ...
    tags = _.uniq(tags, Function.prototype.call.bind(String.prototype.toLowerCase));
    ...
    var saveTags = _.remove(tags, function(tag) {
      return !_(dbTags).findWhere({title: tag});
    });
    ...
    var recipeTags = _.map(tags, function(tag) {
      var rel = {};
      rel[lastId] = _.findWhere(dbTags, {title: tag}).id;
      return rel;
    });
    ...
    var saveIngredients = _.remove(ingredients, function(ingredient) {
      return !_.findWhere(dbIngredients, {title: ingredient.title});
    });
    ...
    var recipeIngredients = _.map(ingredients, function(ingredient) {
      var rel = {};
      var id = _.findWhere(dbIngredients, {title: ingredient.title}).id;
      rel[lastId] = {
        ingredient_id: id,
